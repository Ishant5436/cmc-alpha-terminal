...
@mcp.tool()
async def cmc_screen_momentum(top_n: int = 10, min_volume_usd: float = 100_000_000.0) -> str:
    """
    ...
    if not filtered:
        return "No assets met the minimum volume threshold."
    ...
    lines = [
        ...
        f"**Universe:** {n_total} liquid assets | **Top N Filter:** {len(selected)}",
        "",
...
@mcp.tool()
async def cmc_volatility_regime(symbol: str) -> str:
    """
    ...
    if not quote:
        return f"Error: Asset '{clean_sym}' not found in CoinMarketCap active listings."
...
@mcp.tool()
async def cmc_liquidity_depth(symbol: str) -> str:
    """
    ...
    mcap = quote.get("market_cap_usd", 1.0)
    turnover_ratio = (vol_24h / mcap) if mcap > 0 else 0.0
...
@mcp.tool()
async def cmc_global_macro() -> str:
    """
...
@mcp.tool()
async def cmc_alpha_signals(limit: int = 5) -> str:
    """
    Generate quantitative multi-factor alpha signals (Momentum + Volatility Penalty + Liquidity Guard).
    Returns recommended execution directives (LONG / NEUTRAL / AVOID).
    """
    ...
    for item in listings:
        ...
        mcap = item.get("market_cap_usd", 1.0)
    ...
    for s in selected:
        lines.append(f"| **{s['symbol']}** | ${s['price']:,.2f} | {s['composite']:.2f} | {s['vol']:.4f} | {s['directive']} |")
